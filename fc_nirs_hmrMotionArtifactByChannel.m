...
% Consider tIncMan
% 
% function [tInc,tIncCh,mc_thresh] = fc_nirs_hmrMotionArtifactByChannel(d, fs,  tIncMan, tMotion, tMask, std_thresh, amp_thresh)
% 

function [tInc,tIncCh] = fc_nirs_hmrMotionArtifactByChannel(signal, t,  tIncMan, tMotion, tMask, std_thresh, amp_thresh)
...
[a b]=size(std_sig);
filter_para=ones(1,round(tMask/abs(t(1)-t(2))));
for i=1:size(std_sig,2);
std_sig(:,i)=filter(filter_para,1,std_sig(:,i));
end
tIncCh=ones(a,b);
tIncCh(find((std_sig>0)))=0;
